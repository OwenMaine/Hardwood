...
jQuery(document).ready(function(){
    ...
    if (chart.length) {
        centerChartsNr();
        $(window).resize(debounce(centerChartsNr, 200));

        chartParent.each(function () {
            // Ensure charts are only initialized once using .one()
            $(this).one('doIn', function () {
                $(this).find('.chart').easyPieChart({
                    animate: 1000, // Changed from 2000ms to 1000ms
                    lineWidth: 3,
                    barColor: '#2f2f2f',
                    trackColor: '#dcdcdc',
                    lineCap: false,
            ...
            // Trigger onScreen event (existing onScreen plugin code)
            $(this).onScreen({
                doIn: function () {
                    $(this).trigger('doIn');
                }
